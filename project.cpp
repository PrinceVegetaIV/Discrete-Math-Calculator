...
void bitwise() {
    ...
    if (n == 1) {
        ...
        try {
            int result = evaluateBitwiseExpression(expression);//185 no line
        ...
        cin >> decimal;
        cout << "Binary: " << decimalToBinary(decimal) << endl;//277
    }
    else if (n == 3) {
        string binary;
        cout << "Enter a binary number: ";
        cin >> binary;
        cout << "Decimal: " << binaryToDecimal(binary) << endl;//282
    }
}
...
void set_operation() {
    ...
    vector<int> result;
    if (n == 1) {
        result = setUnion(set1, set2);//323
    }
    else if (n == 2) {
        result = setIntersection(set1, set2);
    }
    else if (n == 3) {
        result = setDifference(set1, set2);//345 no line
    }
    else if (n == 4) {
        result = setSymmetricDifference(set1, set2);//356 no line
    }
...
void num_theory_operation() {
    ...
    cout << "5. Number of Primes up to a Number\n";
    cout << "6. Euler's Totient Function\n";
    cout << "7. Inverse Modulo\n";
    cout << "8. Extended GCD\n\n";
    ...
    if (n == 1) {
        ...
        cout << "Number of primes up to " << num << ": " << primes.size() << endl;
        cout << "Primes: ";
        ...
        cout << "Enter a number: ";
        cin >> num;
        cout << "Euler's Totient: " << eulerTotient(num) << endl;
    }
    else if (n == 7) {
        int a, mod;
        cout << "Enter number and modulus: ";
        cin >> a >> mod;
        cout << "Modular Inverse: " << modInverse(a, mod) << endl;
    }
    else if (n == 8) {
        int a, b, x, y;
        cout << "Enter two numbers: ";
...
void mat_operation() {
    ...
    if (choice == 1 || choice == 2 || choice == 3) {
        ...
        if (choice == 1) {
            vector<vector<int>> result = addMatrices(A, B);//673
            printMatrix(result);
        } else if (choice == 2) {
            vector<vector<int>> result = subtractMatrices(A, B);//686
            printMatrix(result);
        } else if (choice == 3) {
            if (colsA != rowsB) {
            ...
            vector<vector<int>> result = multiplyMatrices(A, B);//699 no line
            printMatrix(result);
        }
    } else if (choice == 4) {
        int rows, cols;
        ...
        vector<vector<int>> result = transposeMatrix(matrix);//715 line
        printMatrix(result);
    } else if (choice == 5) {
        int rows, cols, power;
        ...
        cin >> power;
        vector<vector<int>> result = matrixPower(matrix, power);//742 no line
        printMatrix(result);
    } else if (choice == 6) {
        int rows, cols;
...
vector<int> in_degree(N, 0); // In-degree of vertices
...
// Helper function to detect and store the cycle path
bool detectCycle(int node, int parent, unordered_set<int>& visited, vector<int>& path, vector<int>& cycle) {
    visited.insert(node);
    path.push_back(node);
    ...
    for (size_t i = 0; i < adj[node].size(); ++i) {
        ...
        if (visited.find(neighbor) != visited.end()) {
            // Cycle detected
            // Start from the neighbor node to the end of the path
            ...
            if (detectCycle(neighbor, node, visited, path, cycle)) {
                return true;
            }
        }
    }
...
void cycle() {
    ...
    if (!adj[1].empty()) {  // Starting from node 1 or any valid node
        if (detectCycle(1, -1, visited, path, cycle)) {
            // Print the cycle
            cout << "Cycle detected: ";
...
void dffs(int v, vector<bool> &visited) {
    visited[v] = true;
    for (auto edge : adj[v]) {
        int u = edge.first;
        if (!visited[u]) {
...
// Function to check if all non-isolated vertices are connected
bool isConnected(int V) {
    ...
    if (start_vertex == -1) return true; // No edges in the graph

    dffs(start_vertex, visited);
...
// Function to find Eulerian Circuit or Path
void Euler_circuit_Euler_path(int V) {
...
// Backtracking utility function to find Hamiltonian Path/Circuit
bool hamCycleUtil(vector<int>& path, vector<bool>& visited, int pos, int V) {
    if (pos == V) {
        // For Hamiltonian Circuit, check if the last vertex connects to the first vertex
        return any_of(adj[path[pos - 1]].begin(), adj[path[pos - 1]].end(),
                      [&](const pair<int, int>& p) { return p.first == path[0]; });
    }
    for (const auto& edge : adj[path[pos - 1]]) {
        int v = edge.first;
        if (!visited[v]) {
            visited[v] = true;
            path[pos] = v;
            if (hamCycleUtil(path, visited, pos + 1, V)) {
                return true;
            }
            visited[v] = false;
        }
...
void findHamiltonianCircuit(int V) {
    vector<int> path(V, -1);
    vector<bool> visited(V, false);
    ...
    if (hamCycleUtil(path, visited, 1, V)) {
...
void findHamiltonianPath(int V) {
    vector<int> path(V, -1);
    vector<bool> visited(V, false);
...
void topologicalSortKahns(int V) {
    queue<int> Queue;
    ...
    // Check for cycles (if topologicalOrder does not contain all vertices)
...
void GRAPH() {
    //Storing graph
    cout<<"Store Graph_graph will be connected:\n";
    int n,m;
    cout<<"Enter number of nodes: "; cin>>n;
    cout<<"Enter number of edges: "; cin>>m;
    cout<<"press 1 for directed or 2 for undirected"<<endl;
    int press;
    cin>>press;
    int x,y,w;
    ...
    cout << "5. Cycle chack\n";
    cout<<"6.Euler circuit , Euler path exist or not"<<endl;
    cout<<"7.Hamiltonian paths and circuit  exist or not"<<endl;
    cout<<"8.Top sort"<<endl;
    ...
    if (choice==4) {
        ...
        if (choice==5)cycle();
        if(choice==6){
             Euler_circuit_Euler_path(n);
        }
        if(choice==7){

              findHamiltonianPath(n);
              findHamiltonianCircuit(n);
        }
        if(choice==8)  topologicalSortKahns(n);
...
double logarithm(double number, double base) {
    if (number <= 0 || base <= 1) {
        throw invalid_argument("Number must be positive and base must be greater than 1.");
    }
...
signed main() {
    ...
    while (1) {
        ...
        if (n == 1) {
            ...
            try {
                double result = evaluateExpression(expression);//58  no line
                cout << "Result: " << result << endl;
            } catch (const exception& e) {
                cerr << e.what() << endl;
            }
            ...
            mat_operation();//763 no line
            cout<<"\n";
        }
        else if (n==5) {
            num_theory_operation();//498
            cout<<"\n";
        }
        else if (n==6) {
            combi();
            ...
            if (choice==1) {
                ...
                int a,p; cin>>a>>p;
                cout<<"Result = "<<modpow(a,p,1000000007)<<endl;
            }
            else if (choice==2) {
                try {
                    double num, base;
                    ...
                    cin >> num >> base;
                    double result = logarithm(num, base);
                    cout << "Logarithm of " << num << " with base " << base << " is: " << result << endl;
                }
